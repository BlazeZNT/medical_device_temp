    ...
    methods: {
        ...
        async createEngine() {
            ...
            this.engine.on("roomStreamUpdate", (roomID, updateType, streamList) => {
                this.appendSuccessInfo(`roomStreamUpdate： roomID:${roomID}, updateType: ${updateType}, playStream: ${!!this.playStream}, isPlayingStream: ${this.isPlayingStream}`);
                // 流更新的相关操作, 以及关于断网后的重连出发的流更新
                const stream = streamList.find(item => item.streamID === this.playStreamID)
                console.warn(`stream:${stream}, updateType: ${updateType}, playStream: ${this.playStream}, isPlayingStream: ${this.isPlayingStream}`)
                if(stream) {
                    // 0 为add, 1 为delete
                    if (updateType == 0) {
