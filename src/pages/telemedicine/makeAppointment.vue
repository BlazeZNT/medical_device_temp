			...
			<view class="form">
				<uni-forms label-position="top" label-width="120rpx" :border="false" :modelValue="state.userInfo">
					<view v-if="doctorDataAvailable" class="doctor-info-box">
						<image
							:src="potato[0].image ? 'data:image/jpeg;base64,' + potato[0].image : '/static/doctordemo.png'"
							class="doctor-image" />
						...
						<view class="time-details">
							...
							<view class="doctor-date">{{ `${potato[0].date} ${potato[0].year} ${potato[0].time}` }}
							</view>
						</view>
					</view>
					<view v-else class="column">
						<uni-forms-item label="Tell your health complaints" name="healthComplaints">
							<textarea class="simple-textarea" placeholder="Tell us your symptoms"
								v-model="textInput"></textarea>
							<div class="textarea-content" contenteditable="true" @input="handleTextChange">
								{{ transcript }}</div>
							<button class="record-button" @longpress="ToggleMic" @touchend="touchend">Record</button>
					...
					<view class="column">
						<uni-forms-item label="Select Date" name="date">
							<BasicButton @click="handleClickCalendar"
					...
					<view class="column">
						<uni-forms-item label="Time" class="timeButtons">
							<view class="timeButtons">
								<BasicButton v-for="(time, index) in times" :key="index"
...
<script setup>
	...
	import {
		useAppStore
	} from "@/stores/app"; // Import the store
	...
	const times = ["08:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00", "21:00", "22:00", "22:30", "23:00",
		"23:30"
	];
	...
	const ToggleMic = () => {
		console.log(speechRecModule)
		speechRecModule.startRecord({
			uniCode: 'speechTest'
		}, (res) => {
