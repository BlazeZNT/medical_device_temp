...
export default class ZegoExpressEngine {
     ...
     *
     * 支持版本：3.1.0 及以上。
     * 详情描述：设置视频采集源，用于在不同的视频采集源之间进行切换。
     ...
     *
     * 支持版本：3.1.0 及以上。
     * 详情描述：设置音频采集源，用于在不同的音频采集源之间进行切换。
     ...
     *
     * 支持版本：1.1.0 及以上。
     * 详情描述：此函数用于控制 [ZegoVideoSourceTypeCamera] 使用前置摄像头或者后置摄像头（仅 Android 和 iOS 支持）。
     ...
     * 注意：此函数仅在 ZegoExpressVideo SDK 中有效！
     *
     * @param enable [ZegoVideoSourceTypeCamera] 是否采用前置摄像头；true: 表示使用前置摄像头；false: 表示使用后置摄像头。
     * @param channel 推流通道。
     */
     ...
     * @return 摄像头最大变焦倍数
     */
    getCameraMaxZoomFactor(channel) {
        return ZegoExpressEngineImpl.getInstance().getCameraMaxZoomFactor(channel);
    }
    /**
     * 启动声浪监控，支持开启进阶功能。
     *
     ...
     * @param channel 推流通道
     */
    enableTorch(enable, channel) {
        return ZegoExpressEngineImpl.getInstance().enableTorch(enable, channel);
    }
    /**
     * 是否开启回声消除。
     *
