...
const TAG = '[SimpleExpressEventHandler]';
export class SimpleExpressEventHandler {
    ...
    engineStateUpdate = (state) => {
        zloginfo(`${TAG} engineStateUpdate: ${state}`);
    ...
    publisherStateUpdate = (streamID, state, errorCode, extendedData) => {
        zloginfo(`${TAG} publisherStateUpdate: ${streamID} ${state} ${errorCode} ${JSON.stringify(extendedData)}`);
